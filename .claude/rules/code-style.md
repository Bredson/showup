...
## UI (React) — wzorce z fazy Statystyki zaawansowane (Showup, 2026-07-17)

- **Tekst sr-only to string i18n jak każdy inny — reguła interpunkcji dotyczy go
  W CAŁOŚCI**: review złapał `` `${date}: ${feel}` `` sklejane w JSX TRZECI raz
  w historii tej samej reguły (F6 Unstuck → Progres Showup → tu). Nowa procedura:
  przed napisaniem JAKIEGOKOLWIEK `sr-only` grep po `srPoint`/`srWeek` w i18n
  i skopiuj wzorzec klucza z placeholderami (`'{date}: {feel}'`). Jedyny dozwolony
  literał w JSX to strukturalny separator `join('; ')` między już
  przetłumaczonymi elementami.
- **Sekcja-agregat renderuje się dopiero, gdy coś agreguje**: podsumowanie per
  wariant przy `log.length > 1` — przy jednym teście byłoby duplikatem jedynego
  wiersza listy. Próg w JSX z komentarzem WHY; domena liczy zawsze (tanie),
  warunek to decyzja prezentacji.
- **Słupki danych bez biblioteki: track o stałej wysokości + fill z `height: %`
  inline**: procent derywowany z danych to sankcjonowany inline-style (precedens
  `level-bar-fill`); kształt/kolory w klasach na tokenach. Stan „w toku"
  (bieżący tydzień) = wariant koloru `--primary-soft`, nie inny kształt.
  Zero na słupku zostaje bez etykiety licznika (anty-presja — pusty track
  wystarczy, liczba by dobijała).
- **Wspólna oś dat pod paskiem statystyk = jedna klasa `.stats-axis`**
  (first/last `formatDayShort`), współdzielona między sekcjami zamiast
  per-sekcyjnych kopii.
- **Poziomy skali emoji przez `repeating-linear-gradient` na tokenie
  `--surface-alt`** — linie-prowadnice siatki bez dodatkowych elementów DOM;
  mapowanie poziom→rząd to `Record<Feel, number>` (exhaustiveness z typu).
- **Wariant funkcji etykiet = pełny `switch`, nie `if` + zawężony `switch`**:
  narrowing daje exhaustiveness w obu formach, ale lustrzana struktura z
  sąsiednią funkcją (`outcomeLabel` obok `summaryOutcomeLabel`) czyta się
  szybciej i diffuje czyściej (finding NIT z review).
