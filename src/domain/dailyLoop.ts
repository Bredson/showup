...
// Every transition returns a NEW LegacyDailyEntry — callers persist it via StorageAdapter.

import type { LegacyDailyEntry, Emotion, ISODateTime } from './types';
...
export function setEmotion(entry: LegacyDailyEntry, emotion: Emotion, now: ISODateTime): LegacyDailyEntry {
 ...
 */
export function setIfThen(entry: LegacyDailyEntry, text: string | null, now: ISODateTime): LegacyDailyEntry {
 ...
 */
export function completeEntry(entry: LegacyDailyEntry, reflection: string, now: ISODateTime): LegacyDailyEntry {
 ...
 * LegacyDailyEntry has no step field (data-model: no derivable state persisted), so we infer:
 ...
 */
export function resumeStep(entry: LegacyDailyEntry): LoopStep {
 ...
 */
export function shouldHideIfThenEducation(entries: LegacyDailyEntry[]): boolean {
